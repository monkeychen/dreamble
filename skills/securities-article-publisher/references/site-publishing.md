...
`source: wechat` 的含义是“首发于微信公众号”，不是“已放入公众号草稿箱”。

- 微信文章已真实公开发布，且网站文章是备份：生成网站版本时加 `--source-wechat`。
- 微信文章只有草稿、用户希望保持“公众号首发”：生成网站版本时加 `--draft`，不要执行网站发布命令；等用户确认公众号已公开后，再移除 `draft: true`、加 `source: wechat` 并发布。
- 用户明确要求网站独立或同步首发：不写 `source`，可直接发布网站。

不能仅凭公众号草稿保存成功推断文章已经公开。
...
按状态追加 `--draft` 或 `--source-wechat`。脚本会：

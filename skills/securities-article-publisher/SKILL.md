...
2. 本次需要新做证券研究时，必须加载 `$securities-analyst`，使用其个股、行业或宏观框架；不要让用户选择内部研究引擎。
   - 当前环境没有 `$securities-analyst` 时，停止新研究并提示启用该 Skill；可以继续处理用户已经提供的完整主稿。
   - 内置 [stock-analyzer-01.md](references/stock-analyzer-01.md) 只作为历史基准和补充检查清单，不再作为独立生产研究内核。
   - 只有用户明确要求对照实验时，才在不发布的前提下运行原始 Prompt 基准，并与 `$securities-analyst` 结果比较。
   - 用户提供已完成主稿且只要求配图或发布时，跳过证券研究。
...
1. 使用 `$securities-analyst` 完成研究。原始 Prompt 只用于明确要求的离线对照实验，当前用户要求、仓库规则和系统约束优先级更高。
...
维护本仓库时，根目录 `prompts/stock-analyzer-01.md` 是历史基准的同步副本。修改任一副本后，必须同步另一份并运行 `scripts/test_packaging.py`；对照实验始终以内置 reference 为准，生产研究仍使用 `$securities-analyst`。

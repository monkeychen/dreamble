...
```bash
# 读本文件时已知其绝对路径，取其所在目录即可：
KB_SCRIPTS="<本文件所在目录>/scripts"
# 本机已验证值：~/.workbuddy/skills/ima-skills/knowledge-base/scripts
# 若不确定，运行时自动查找：
KB_SCRIPTS="$(find "$HOME/.workbuddy/skills" "$HOME/.claude/skills" -type d -path "*ima-skill*/knowledge-base/scripts" 2>/dev/null | head -1)"
# ⛔ 查找为空 → 停止，报告「脚本目录缺失，skill 安装不完整」，不要继续执行上传流程
```
...
PREFLIGHT=$(node "$KB_SCRIPTS/preflight-check.cjs" \
...
node "$KB_SCRIPTS/cos-upload.cjs" \
  --file "/path/to/report.pdf" \

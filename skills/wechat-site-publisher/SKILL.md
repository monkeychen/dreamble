...
| `baoyu-cover-image` | 条件必需 | 仅用户未提供封面时才需要；缺失且无可用封面时，停止依赖封面的渠道并报错，**不挂起等待用户提供封面**，也不得声称已生成 |
...
本 Skill 可能被定时任务、流水线或上游 Agent 在无人在场的上下文里调用。此时任何「等用户回答」的环节都会让任务挂死——不会有人回答，任务也永远不会结束。因此按下面的原则区分两类动作：

1. **内部决策一律自主完成，不询问。** 包括插图张数与插入位置、封面比例与风格、提示词措辞、`title` / `summary` 补写、slug、标签、公众号主题。判断有偏差可以在下一轮修正，挂住则什么都不会发生。所有自主决策在「完成报告」中逐项披露，供事后复核与纠偏。
2. **外部状态变更需要授权，但授权来自调用方的请求文本本身。** 公众号草稿写入、Git 推送、网站部署这三类动作：调用方请求或自动化 prompt 已写明范围时，按该范围直接执行，不重复确认；未写明范围时**停止该渠道、保留已生成的本地产物并报告原因**，绝不挂起等待回答。
3. **失败即退出（fail fast），不重试、不等输入。** 依赖缺失、授权缺失、配置缺失一律按此处理：写明原因、保留产物、退出。无人值守场景下，一次带明确原因的失败退出远好过一个无限期挂起的进程，前者可报警可重跑，后者只是白白占着。
4. **被调 Skill 的交互式流程一律跳过。** `baoyu-cover-image`、`baoyu-article-illustrator` 等若含偏好选择、风格确认或首次设置引导，直接采用其默认值或本 Skill 给定的参数继续；不进入交互式提问。其提示词留档等其他要求照常遵守。
...
3. 从调用方的请求文本自行判定交付范围：只准备本地材料、保存公众号草稿、准备网站版本或发布网站。**不为此询问用户**；请求未写明时按「默认双渠道」执行，外部写入仍受「发布授权」约束。
...
- 两个渠道均完成并核验前保留工作目录。全部渠道成功并核验后，默认清理本次工作目录 `tmp/wechat-site-publisher/YYYY-MM-DD-<slug>/`；用户明确说不要清理时才保留。
- **任一渠道失败或未获发布授权时一律保留工作目录**，让人或下一次运行能从断点续跑。无人值守场景下尤其不能因为任务中断就丢掉已生成的主稿和图片。
...
1. 封面资产是必需项。用户未提供可用封面时，调用 `$baoyu-cover-image`，以 `source.md` 为输入，默认取微信公众号宽封面 `2.35:1`，最终图片放到 `imgs/cover.png`。比例、风格、配色等偏好**自主按其默认值决定，不做交互式确认**（见「自动化与无人值守执行」第 4 条）；提示词留档要求照常遵守。
2. 正文插图不是默认凑数量，张数按以下顺序自主决定，**全程不询问用户**：

   - **用户显式指定张数** → 按用户说的来，包括超过 3 张的情形——用户已给出明确指令，无需再确认。
   - **用户未指定** → 基于「接收原稿与准备主稿」第 2 条通读的理解自行判断，**默认 0 张**；只有某处内容仅靠文字不易理解时才增加，**最多 3 张**。

   判断依据是那段内容本身的表达难度（结构关系、对比差异、演变过程、运作机制等），**与文章题材无关**——研报、随笔、教程一视同仁。**直接生成，不在动手前征求用户意见**；每张图表达什么、插在哪一节之后，在「完成报告」中列出，供事后复核与纠偏。
...
- 调用方的请求或自动化 prompt 已明确写明范围（“保存到公众号草稿”“同步/发布到网站”等）时，按该范围执行，**不重复确认**。这是无人值守场景下的常态路径。
- 请求未写明外部发布范围时，按有无人在场分两种处理：

  - **无人在场**（定时任务、流水线、上游 Agent 委派、调用方 prompt 声明自动执行）：停止外部发布，保留 `article.md`、`imgs/`、`site-post/` 等全部本地产物，在报告中标记“待授权，未执行外部发布”，**不挂起等待回答**。
  - **人在场的交互会话**：可在发布前集中展示标题、摘要、封面、目标公众号账号、公众号主题和两个目标渠道，询问一次授权。
...
2. 逐项自检生成的 `index.md`：frontmatter 字段完整、图片引用路径可解析、日期与 slug 一致、摘要和标签已填写。无人值守时这一步由本 Skill 自己核验，**不转交人工**；发现不符就修正后重新生成。

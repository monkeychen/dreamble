...
**变更**：先更新 `AGENTS.md` 建立统计边界，再新增 Nginx 独立访问日志与 GoAccess 私有报告。统计页面使用 HTTP Basic Auth，报告每 5 分钟更新；URL 查询参数被丢弃，IP 使用最高级别匿名化并隐藏访客主机面板，统计页自身不记日志。原始日志每日轮转保留 30 天，月度聚合报告保存在服务器私有目录。新增幂等凭据生成、nginx 配置补丁、OpenCloudOS/Debian 安装、systemd timer、凭据查看和回归测试。
**验证**：完整验收通过；配置脚本在真实 OpenCloudOS 服务器安装并启用 GoAccess，nginx 配置检查通过。未认证访问 `/stats/` 返回 401，认证访问返回 200；真实浏览器确认统计面板可读且站点文章页未增加统计脚本，定时任务和日志轮转配置均正常。
**遗留**：当前只统计页面访问，不记录 md2wechat 的复制、导出等工具行为；是否增加 Umami 事件统计待积累一段时间后再决定。

### 2026-07-27 · 发布命令内置源码自动同步

**动机**：虽然 `AGENTS.md` 已授权站点任务自动闭环，但直接执行 `npm run publish` 仍只部署、不提交源码；使用者必须依赖外部执行者记得先 commit/push，命令本身没有兑现“一步发布”。
**变更**：发布链路调整为“完整验收 → 只提交 `site/` 变更 → 推送 `origin/main` → 服务器同步 → 线上健康检查”。新增独立源码同步脚本，强制要求 `main` 分支，通过 Git pathspec 隔离提交范围，保留仓库其他目录的已暂存和未暂存改动；提交或推送失败时在部署前停止。默认提交信息可由 `PUBLISH_COMMIT_MESSAGE` 覆盖。
**验证**：新增临时本地仓库与 bare remote 端到端测试，确认修改、删除和新增的 `site/` 文件被提交并推送，仓库外部已暂存改动仍保持原状；再用本次未提交修改直接执行 `npm run publish`，确认命令自动生成提交、推送 `origin/main`、完成 rsync 部署和线上健康检查。
**遗留**：无。

### 2026-07-27 · 清理 ChatGPT 内容引用占位符

**动机**：从 ChatGPT 复制 Markdown 时可能混入 `::chatgpt-content-reference{...}` 内部引用占位符；md2wechat 原先把它当普通段落渲染，复制到微信公众号后会出现在文章尾部。
**变更**：Markdown 预处理阶段自动移除正文中的独立 ChatGPT 内容引用占位行；使用代码围栏状态识别保护代码块，文章确实需要展示该语法示例时不会被误删。预览、富文本复制、HTML 导出和打印/PDF 共用同一清理结果。
**验证**：新增回归测试覆盖“正文清理、代码示例保留”；完整验收通过，并用真实浏览器将含占位符的 Markdown 复制后读取剪贴板 HTML 与纯文本，确认两种格式均不再包含占位符且正文结构保持完整。
**遗留**：无。

### 2026-07-26 · 扩大站点自动发布授权

**动机**：原规则只允许文章和作品自动提交、推送和发布，代码、配置、脚本、文档仍需站主逐项下令；这会把已经验证通过的站点任务拆成多轮人工命令，与站主希望的自动闭环不一致。
**变更**：`AGENTS.md` 的长期授权扩大到当前任务涉及的全部 `site/` 文件，验证通过后自动 commit、推送 `origin/main` 并发布；仍禁止夹带其他任务和 `site/` 之外的无关改动。同步更新 README 与部署手册，并从 `publish.sh` 删除源码 pull/commit/push：源码交付由任务闭环负责，发布脚本只验收、同步和检查线上状态，避免干净工作区因“无内容可提交”把成功发布误报为失败。
**验证**：`npm run verify` 全过；`bash -n scripts/publish.sh` 通过；源码推送后执行 `npm run publish`，rsync 同步和线上健康检查成功，命令以 0 退出。
**遗留**：无。

### 2026-07-26 · md2wechat 完整工作流升级

**动机**：首版把“类似参考站点”错误收缩为三套配色和复制链路，只验证了 MVP；三套主题共用同一版式，也不足以支撑站主日常从写作到公众号发布的完整任务。
**变更**：先更新 `AGENTS.md` 和完整设计规范，明确 md2wechat 不得再以 MVP 交付。工具现支持 Markdown/TXT 导入、Markdown/内联 HTML 导出、打印/PDF、18 个格式与文档操作、本地图片内嵌、外链转脚注、目录、公式、注音和 GFM；提供 12 套标题/引用/列表/表格等结构均有差异的正文主题、7 套独立代码主题、40 类常用代码语言按需加载，以及主题色/字号/行高/段距/字体/代码换行调整；补齐双栏/单面板/全屏/同步滚动、本地草稿与设置恢复。富文本复制继续使用安全 DOM 序列化，并将公式所需样式内联后移除 class/id。
**验证**：36 项单测通过，Astro 检查 0 error / 0 warning / 0 hint，静态构建无大块告警。Playwright 实测 12 套正文主题产生 12 组独立结构样式、7 套代码主题背景和语法色均不同，Rust 等代码语言可按需高亮；导入、MD/HTML 下载、打印窗口、本地图片、外链脚注、格式按钮、设置持久化、全屏和长文同步滚动均闭环。1440px 双栏视觉正常；375px 默认仅编辑并可一步切到预览，页面宽度与视口同为 375px。实际富文本复制再粘贴后，标题、列表、引用、表格、代码和脚注结构完整，无原始 `<section>` 文本、无 class/id；脚本、iframe、事件属性和用户样式均被清理。
**遗留**：不接入公众号草稿箱直传或第三方图床凭据；这两项会改变纯静态站点的账号与安全边界，需要独立立项。

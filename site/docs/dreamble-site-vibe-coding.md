...
### 2026-07-26 · 修复 md2wechat 复制根标签

**动机**：富文本复制结果粘贴到微信公众号编辑器后，正文顶部会出现一段原始 `<section>` 标签，用户还需手动删除，违背“一次复制即可粘贴”的核心体验。
**变更**：不再用字符串拼接预览区的根节点样式，改由浏览器 DOM/CSSOM 写入 `style.cssText` 并通过 `outerHTML` 序列化。字体族中的双引号因此会自动转义为合法 HTML 实体，不再截断 `style` 属性。
**验证**：Astro 检查 0 error / 0 warning / 0 hint；Playwright 读取真实剪贴板 `text/html`，确认字体引号已转义且不存在错误属性；再将剪贴板实际粘贴到可编辑区域，标题、段落、列表、引用、表格、代码和链接均保持富文本结构，顶部未出现原始 `<section>` 文本。
**遗留**：无。

### 2026-07-26 · 新增工具区与 md2wechat

**动机**：站点已有文章与作品，但缺少承载在线小工具的统一入口。参考 `wechat.jeffjade.com` 的双栏 Markdown 排版体验，先解决最核心的任务：写 Markdown、实时确认微信公众号样式、复制后直接粘贴。
**变更**：顶部新增“工具”菜单；`/tools/` 使用左侧工具栏并首发 `md2wechat`。编辑器支持 GFM Markdown、实时预览、旧杂志/简约蓝/清新绿三套主题、浏览器本地自动保存、Tab 缩进，以及带内联样式的 HTML/纯文本双格式复制。输入先经 DOMPurify 清理，内容不上传服务器；图片上传和公众号草稿箱直传因需要图床或账号凭据，本版明确不做。
**验证**：`npm run verify` 全过，Astro 检查 0 error / 0 warning / 0 hint，28 项单测通过，11 个静态页面生成。Playwright 实测 1440px 桌面与 375px 手机布局；Markdown 标题、粗体和表格可实时渲染，三套主题可切换，刷新后草稿与主题仍保留，富文本剪贴板复制成功；注入的 `script` 与事件属性被清理，未进入预览。
**遗留**：后续增加工具时复用左侧工具栏；如确有图片上传或草稿箱需求，需先单独设计凭据与服务器边界。

...
**目录即数据库，放文件即发表。** 没有站点内容管理后台、没有数据库、没有在线 CMS——把一个 markdown 目录放进约定位置，跑一条命令，就完成发表。约定不靠自觉：目录命名、日期一致性、slug 唯一性和 frontmatter 都由构建命令强制校验，填错会直接报错并指明原因。
...
顶部“工具”进入 `/tools/`。工具区使用独立侧边栏，当前提供 `md2wechat`：

- 新建或导入 Markdown，使用格式工具栏完成标题、列表、引用、表格、链接、图片、代码、目录、脚注、公式和注音
- 12 套完整正文主题与 7 套代码主题分开选择；还可调整主题色、字号、行高、段距和字体
- 支持双栏、仅编辑、仅预览、全屏和同步滚动；手机默认单面板，避免来回缩放
- 可导出 Markdown、带排版 HTML、打印/PDF；本地图片可直接内嵌，远程图片使用 HTTPS URL
- 草稿、主题和设置自动保存在当前浏览器，内容不会上传服务器
- 点击“复制到微信公众号”，再到公众号编辑器粘贴即可保留内联富文本样式

纯静态站点不保存公众号账号或图床凭据，因此不接入公众号草稿箱直传，也不代管远程图片。

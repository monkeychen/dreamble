...
显示各市场股票数、日期范围、最近信号（读上次 screen 的缓存，秒级返回）。

### `update` — 增量更新数据

```bash
./stockfunnel update              # 更新全市场
./stockfunnel update -m star      # 只下载科创板的新数据
```

只拉取最后一个交易日之后的新数据，通常几秒到几十秒。自动检测新增股票并补全历史。

- 每只股票从**自己的**最后日期起增量拉取：某只更新失败不会留下永久缺口，下次 update 自动补齐
- 更新前用多只大票探针确认数据源已发布当日数据，未发布则跳过（非交易日/盘后数据未出时快速返回）
- `-m` 只限制**下载范围**；合并 `daily.parquet` 始终包含全部市场，不会污染其他市场的数据
...
清空该市场范围的已有分片，从头下载。首次使用或数据损坏时用。中断后重跑 `rebuild` 可续传（已重新下载的分片不会重复下载）。
...
注意：`-m` 只影响 update/rebuild 的**下载范围**和 screen/backtest 的**统计范围**；本地主数据文件 `daily.parquet` 始终由全部分片合并生成，与 `-m` 无关。

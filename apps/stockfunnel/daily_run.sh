#!/bin/bash
# stockfunnel 每日任务
# 由 launchd（com.dreamble.stockfunnel）每天 19:00 触发，也可手动执行：
#   ./daily_run.sh
# 流程：增量更新 → 指数择时信号（主策略）→ 筛选最近 5 天信号（观察）
# 产物：output/daily_timing.txt + output/daily_screen.txt
...
START_TS=$(date +%s)
{
    ...
    # 2. 指数择时信号（主策略）：翻转日才有操作
    TIMING_OUTPUT=$(./stockfunnel timing 2>&1)
    TIMING_RC=$?
    if [ $TIMING_RC -eq 0 ]; then
        {
            echo "===== stockfunnel 指数择时 $(date '+%Y-%m-%d %H:%M') ====="
            echo ""
            echo "$TIMING_OUTPUT"
        } > "$TIMING_FILE"
        if echo "$TIMING_OUTPUT" | grep -q "今日翻转"; then
            echo "[timing] ⚡ 有翻转信号，明日开盘需操作！详见 daily_timing.txt"
        else
            echo "[timing] 无翻转，无需操作"
        fi
    else
        echo "[timing] 退出码 ${TIMING_RC}（失败）"
        echo "$TIMING_OUTPUT" | tail -5 | sed 's/^/[timing] /'
    fi
